  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card) => {
        ...
        // For risk zones, positive change is bad
        const trendColor = card.isRisk
          ? isPositive ? 'text-[var(--alert-red)]' : isNegative ? 'text-[var(--safe-green)]' : 'text-[var(--text-secondary)]'
          : isPositive ? 'text-[var(--safe-green)]' : isNegative ? 'text-[var(--alert-red)]' : 'text-[var(--text-secondary)]'
          ...
          <Card key={card.label} padding="sm" className="relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {card.subtitle && (
                  <p className="text-xs text-[var(--text-secondary)] mt-0.5">{card.subtitle}</p>
                )}
              </div>
            ...
            <div className={`flex items-center gap-1 mt-2 text-xs ${trendColor}`}>
              ...
              <span>
                {change === 0 ? 'No change' : `${Math.abs(change)}% ${isPositive ? 'increase' : 'decrease'}`}
              </span>
            </div>
          </Card>
        )
