      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            {/* Category Badge */}
            <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full ${categoryStyle.bg} ${categoryStyle.text}`}>
              {categoryStyle.icon}
              {post.hazardCategory}
            </span>
            ...
            {isResolved ? (
              ...
              <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-bold rounded-full bg-red-600 text-white animate-pulse">
          ...
          <div className="relative">
            ...
            {showActions && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-[var(--border-soft)] py-1 z-20 min-w-[140px]">
                  <button
                    onClick={handleShare}
        ...
        {/* Title */}
        <h3 className={`text-lg font-bold mb-2 leading-tight ${isResolved ? 'text-gray-500' : 'text-[var(--text-primary)]'}`}>
          {post.title}
        </h3>

        {/* Media Placeholder */}
        {post.mediaUrl && (
          <div className="mb-3 rounded-lg overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 h-44 flex items-center justify-center">
        ...
        {/* Description */}
        <p className={`text-sm mb-3 leading-relaxed line-clamp-2 ${isResolved ? 'text-gray-400' : 'text-[var(--text-secondary)]'}`}>
          {post.description}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs mb-3">
          <span className="flex items-center gap-1 text-[var(--text-secondary)]">
            ...
            <span className="truncate max-w-[180px]">{post.location.name}</span>
          </span>
          <span className="flex items-center gap-1 text-[var(--text-secondary)]">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Tooltip content={`${Math.round(post.authorReliabilityScore * 100)}% community validation rate`}>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[var(--border-soft)]">
          ...
          <div className="flex items-center">
            <button
              ...
              disabled={isVoting}
              className={`
                p-2 rounded-l-lg border transition-all
                ${userVote === 'up' 
            ...
            <div className={`
              px-3 py-2 border-y text-center font-bold text-sm min-w-[3rem]
            ...
            <button
              ...
              disabled={isVoting}
              className={`
                p-2 rounded-r-lg border transition-all
                ${userVote === 'down' 
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-[var(--text-secondary)] transition-all"
          ...
          >
            ...
            <span className="text-xs text-gray-400 hidden sm:inline">updates</span>
          </button>
        </div>
      </div>
    </article>
