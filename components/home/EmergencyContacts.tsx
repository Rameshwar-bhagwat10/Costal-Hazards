      ...
      <div className="grid grid-cols-2 gap-2">
        {emergencyContacts.map((contact) => (
          <a
            ...
            className="flex items-center gap-2 p-2.5 bg-white/10 rounded-lg hover:bg-white/20 transition-colors group"
          >
            <span className="text-lg">{contact.icon}</span>
            <div className="min-w-0">
              <p className="text-xs text-slate-300 truncate">{contact.name}</p>
              <p className="text-sm font-semibold group-hover:text-blue-300 transition-colors">
                {contact.number}
              </p>
            </div>
          </a>
